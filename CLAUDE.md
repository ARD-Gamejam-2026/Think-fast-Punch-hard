# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

"Think fast, Punch hard" — a game for the ARD Game Jam 2026, built with **Unity 6000.5.9f1** using the **Universal Render Pipeline (URP)** and the **new Input System** (`com.unity.inputsystem`). The fighter half (movement, attacks, AP/Flow economy, health, HUD, and an autonomous AI opponent) and the quiz half are both implemented — see **Documentation** below before exploring the code.

Links:
- Design board: https://miro.com/app/board/uXjVHv0lDhg=/
- Jam page: https://itch.io/jam/-ard-game-jam-2026
- Game page: https://rhinocerosgamesproduction.itch.io/think-fast-punch-hard

## Documentation

Read these before working on gameplay — they explain how the systems fit together
and which seams to build against:

- `Assets/Scripts/README.md` — **fighter half**: movement, attacks, AP/Flow economy,
  health, HUD, the AI opponent (chasing, platform climbing, attacking), the
  `IDamageable`/`HitInfo` and `IFighterMotor` contracts, the `RiddleRewards` seam to
  the quiz and the `RoundEvents` seam to match flow, debug keys, and tuning defaults.
  **Read its "Retuning the fighters" section before changing jump height, run speed or
  gravity** — the opponent's platform routes depend on those and nothing enforces it.
- `Assets/Quiz/README.md` — **quiz half**: questions, sessions, the endless `QuizFlow`
  driver, and the `QuestionResolved`/`QuestionAnswered` events other systems hook into.
- `Assets/Scripts/Tutorial/README.md` — **the tutorial scene**: the step machine, the
  training dummy, and the two build-order rules its generator depends on.
- `GAME_DESIGN.md` — game design overview and decisions. Gitignored, local only, so
  it may be absent in a fresh clone.

Headless compile check without opening Unity (the editor holds an exclusive project
lock, so `-batchmode` fails while it is open):

```
powershell -NoProfile -ExecutionPolicy Bypass -File tools/compile-check.ps1
```

## Commands

Unity is installed via Unity Hub. The editor binary for CLI use — **Windows** (this
machine), with the macOS path kept for anyone on a Mac:

```
UNITY="C:/Program Files/Unity/Hub/Editor/6000.5.9f1/Editor/Unity.exe"     # Windows
UNITY="/Applications/Unity/Hub/Editor/6000.5.9f1/Unity.app/Contents/MacOS/Unity"  # macOS
```

**Any `Tools > Think Fast` command can be run headlessly while the editor is closed**,
which is the fastest way to regenerate a scene without a round trip:

```
"$UNITY" -batchmode -quit -nographics -projectPath . -logFile - \
  -executeMethod ThinkFast.TutorialEditor.TutorialSceneBuilder.Build
```

The editor holds an exclusive lock, so this fails while Unity is open — use
`tools/compile-check.ps1` for a syntax check in that case. Builders that call
`EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo()` must guard it with
`!Application.isBatchMode`, or the headless run hangs on a prompt nobody can answer.

Run EditMode tests (batch mode; project must not be open in the editor at the same time):

```
"$UNITY" -batchmode -projectPath . -runTests -testPlatform EditMode -testResults results.xml -logFile -
```

Run PlayMode tests: same command with `-testPlatform PlayMode`. Filter to a single test with `-testFilter "Namespace.Class.TestName"`.

Headless build check (compile without opening the editor):

```
"$UNITY" -batchmode -quit -projectPath . -logFile -
```

There is no lint step; C# compilation errors surface in the Unity log / `Logs/`.

## Structure & Conventions

- `Assets/Scenes/SampleScene.unity` — registered in Build Settings. `Assets/Scenes/PlayerControllerTest.unity` is the fighter test rig, generated by `Tools > Think Fast > Build PlayerController Test Scene`. `Assets/Scenes/Scene_Tutorial.unity` is generated in full by `Tools > Think Fast > Build Tutorial Scene`; nothing in it is hand-placed, so it is rebuilt from scratch rather than edited.
- `Assets/Settings/` — URP render pipeline assets: separate `PC_RPAsset`/`PC_Renderer` and `Mobile_RPAsset`/`Mobile_Renderer` quality tiers, plus the default volume profile.
- `Assets/InputSystem_Actions.inputactions` — input action map (Player + UI action maps from the template). Use the Input System package for all input, not the legacy `Input` class.
- `Assets/TutorialInfo/` — Unity template readme scaffolding; safe to delete when the project gets real content.
- The quiz uses asmdefs (`Quiz`, `Quiz.EditModeTests`); everything else compiles into `Assembly-CSharp`. Unity only allows `Assembly-CSharp` to reference an asmdef, never the reverse, so fighter code can call quiz code but not vice versa.

Unity-specific rules that matter here:
- Every asset and folder under `Assets/` has a paired `.meta` file — always move/rename/delete them together, and commit `.meta` files with their assets.
- `Library/`, `Logs/`, `UserSettings/`, and the generated `.csproj`/`.slnx` files are build artifacts (gitignored); never edit them by hand.
- Scene files and most `ProjectSettings/` assets are Unity-serialized YAML — prefer making scene/settings changes via small editor scripts or by minimal targeted YAML edits, and avoid reformatting these files.
